...
def get_reservations_report(
    ...
    query = db.query(
        ...
        Reservation, Showtime.id == Reservation.showtime_id
    ).join(
    ...
    if start_date:
        query = query.filter(Showtime.start_time >= datetime.combine(start_date, datetime.min.time()))
    if end_date:
        query = query.filter(Showtime.start_time <= datetime.combine(end_date, datetime.max.time()))
    
    query = query.group_by(
        Showtime.id, Movie.title, Showtime.start_time, 
        Showtime.hall_number, Showtime.total_seats, Showtime.price
    )
    ...
    for r in results:
        capacity_percentage = (r.reserved_seats / r.total_seats * 100) if r.total_seats > 0 else 0
