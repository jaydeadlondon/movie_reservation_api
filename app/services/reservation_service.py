...
class ReservationService:
    ...
    @staticmethod
    def reserve_seats(
        db: Session, user_id: int, showtime_id: int, seat_ids: List[int]
    ) -> List[Reservation]:
        ...
        seats = (
            db.query(Seat)
            .filter(and_(Seat.id.in_(seat_ids), Seat.showtime_id == showtime_id))
            .with_for_update()
            .all()
        )
        ...
        if len(seats) != len(seat_ids):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Some seats not found"
            )
    ...
    def get_user_reservations(db: Session, user_id: int, upcoming_only: bool = False):
        ...
        if upcoming_only:
            query = query.join(Showtime).filter(Showtime.start_time > datetime.utcnow())
